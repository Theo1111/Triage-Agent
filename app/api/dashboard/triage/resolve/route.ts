...
export async function POST(req: NextRequest) {
  try {
    ...
    const item = await resolveTriageItem(triageItemId);
    ...
    await logEvent({
      ...
      action: `Resolved triage item ${triageItemId} by ${actorLabel} via dashboard`,
    });

    await syncTriageItemToSlack(item, `✅ *Status:* Resolved by ${actorLabel} (via dashboard)`);
