...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    // Derive slackAction from the current classification without re-running routing.
    const classification = await getCurrentClassification(inboundEmailId);
    if (!classification) {
      return NextResponse.json(
        { error: `No classification found for email ${inboundEmailId}. Run classify-email first.` },
        { status: 404 }
      );
    }
    ...
    if (classification.urgency_level === "not_relevant" || routingRecommendation?.route_type === "ignore") {
      slackAction = "ignored";
    } else if (
    ...
    if (!triageItem) {
      return NextResponse.json({
        success: true,
        created: false,
        reason: "Email is not_relevant — no triage item created",
        slackAction,
      });
    }
    ...
    console.error(`[triage/create-for-email] Error for ${inboundEmailId}:`, message);

    if (message.toLowerCase().includes("not found")) {
      return NextResponse.json({ error: message }, { status: 404 });
    }
    return NextResponse.json({ error: "Failed to create triage item", details: message }, { status: 500 });
  }
}
